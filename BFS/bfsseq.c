...
void run_bfs_seq(int no_of_nodes, Node *d_graph_nodes, int edge_list_size,
...
void BFSGraph( int argc, char** argv)
{
  ...
  run_bfs_seq(no_of_nodes,h_graph_nodes,edge_list_size,h_graph_edges,
  ...
  //int k=0;
	...
	FILE *fpo = fopen("resultseq.txt","w");
	for(int i=0;i<no_of_nodes;i++)
	fprintf(fpo,"%d) cost:%d\n",i,h_cost[i]);
	fclose(fpo);
	printf("Result stored in resultseq.txt\n");
